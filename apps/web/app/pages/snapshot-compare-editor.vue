    ...
    <div class="space-y-3 rounded border p-3 sticky top-4">
      ...
      <div v-if="versions.length" class="space-y-2">
        <div
          ...
          :key="version.version"
          class="flex items-center justify-between gap-3 rounded border px-3 py-2 cursor-pointer"
          :class="{
            'border-blue-500 bg-blue-50': currentVersion === version.version,
            'hover:bg-gray-50': currentVersion !== version.version,
          }"
        ...
        >
          ...
          <div class="flex items-center gap-3">
            ...
            <Button
              ...
              @click.stop="revert(version.version)"
            >
              恢复
            </Button>
          </div>
...
import Snapshot, { watchPreviewContent } from '~/lib/snapshot-extension';
import type { SnapshotVersion, SnapshotStorage } from '~/lib/snapshot-extension';
...
let stopWatchPreview: (() => void) | null = null;
...
const previewVersion = (versionNumber: number) => {
...
onMounted(() => {
  stopWatchPreview = watchPreviewContent(provider, (content) => {
